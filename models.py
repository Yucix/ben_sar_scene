...
class DSDL(nn.Module):
    ...
    def forward(self, optical, sar, semantic_vectors, sar_nodes, node_mask, target=None, update_scene_counts=False):
        ...
        feature = self.fusion_alpha * f_opt + (1.0 - self.fusion_alpha) * f_sar

        scene_logits = self.scene_head(feature)
        scene_probs = torch.softmax(scene_logits, dim=1)

        semantic = torch.matmul(semantic_vectors, self.W1)
        semantic = self.relu(semantic)
        base_semantic = torch.matmul(semantic, self.W2)
        ...
        if self.training and update_scene_counts and target is not None:
            pred_scenes = torch.argmax(scene_probs.detach(), dim=1)
            with torch.no_grad():
                for b in range(bsz):
                    scene_idx = pred_scenes[b].item()
                    y = target[b].float().unsqueeze(1)
                    self.scene_cooccur_counts[scene_idx] += torch.matmul(y, y.transpose(0, 1))

        p_list = []
        for k in range(self.num_scenes):
            ck = self.scene_cooccur_counts[k]
            diag = torch.diag(ck).clamp(min=1.0)
            pk = ck / diag.unsqueeze(1)
            row_sum = pk.sum(dim=1, keepdim=True).clamp_min(1e-6)
            pk = pk / row_sum
            p_list.append(pk)

        p_all = torch.stack(p_list, dim=0)
        p_i = torch.einsum("bk,kij->bij", scene_probs, p_all)

        base_semantic_batched = base_semantic.unsqueeze(0).expand(bsz, -1, -1)
        dynamic_semantic = torch.bmm(p_i, base_semantic_batched)
        ...
        a_mat = torch.bmm(dynamic_semantic, dynamic_semantic.transpose(1, 2)) + eye_matrix
        b_rhs = torch.bmm(dynamic_semantic, feature.unsqueeze(2))
        score = torch.linalg.solve(a_mat, b_rhs).squeeze(2)
