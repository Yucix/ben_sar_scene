...
class DSDL(nn.Module):
    ...
    def __init__(
        ...
        # Initialize as identity so early behavior is close to original DSDL.
        init_counts = torch.eye(num_classes, dtype=torch.float32).unsqueeze(0).repeat(
            self.num_scenes, 1, 1
        )
    ...
    def forward(
        ...
        if self.training and update_scene_counts and target is not None:
            ...
            with torch.no_grad():
                for b in range(bsz):
                    ...
                    self.scene_cooccur_counts[scene_idx] += torch.matmul(y, y.t())
        ...
        dynamic_semantic = torch.bmm(p_i, base_semantic_batched)  # [B, C, 2048]
        ...
        eye_matrix = (
            self.alpha
            * torch.eye(cls_num, device=device).unsqueeze(0).expand(bsz, -1, -1)
        )
